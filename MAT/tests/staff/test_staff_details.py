...
from MAT.apps.authentication.models import CohortMembership, User
...
class TestStaffDetails():
    ...
    @pytest.mark.django_db
    def test_single_staff_details_with_cohort(self, client, new_admin_user, new_cohort, get_or_create_token):
        token = get_or_create_token
        client.credentials(HTTP_AUTHORIZATION='Bearer ' + token)
        new_admin_user.save()
        new_cohort.save()
        membership = CohortMembership(
            user=new_admin_user, cohort=new_cohort, current_cohort=True)
        membership.save()
        url = reverse('staff:staff_details', kwargs={'id': new_admin_user.id})
        response = client.get(url)
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data.get('first_name') == new_admin_user.first_name
        assert cohort_data['cohort_name'] == "MC01"
    ...
    @pytest.mark.django_db
    def test_get_tm_profile_details_succeeds(self, client, new_admin_user, new_cohort, get_or_create_token):
        token = get_or_create_token
        client.credentials(HTTP_AUTHORIZATION='Bearer ' + token)
        new_cohort.save()
        # new_admin_user.cohort = new_cohort
        new_admin_user.save()
        membership = CohortMembership(
            user=new_admin_user, cohort=new_cohort, current_cohort=True)
        membership.save()
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data.get('first_name') == new_admin_user.first_name
        assert response.data.get('username') == "whack"
